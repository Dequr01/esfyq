...
const porsche911 = '/models/porsche.glb'
const HEADLIGHT_COLOR = new THREE.Color(0xfff4e0)
...
function Model({ url, isMobile, metalness, roughness, envMapIntensity, lightsOn }) {
    ...
    clone.traverse((node) => {
      if (node.isMesh) {
        ...
        mats.forEach((mat) => {
          ...
          const targetMats = [
            'light_chrome-material.001',
            'light_chrome-material.002',
            'light_chrome-material.003',
            'light_chrome-material.004'
          ];
          // We check if either the node's name or the material's name matches our targets.
          const isTargetLight = targetMats.some(t => nodeName.includes(t) || matName.includes(t));

          if (isTargetLight) {
            if (!node.userData.hasLightAttached) {
               const ptLight = new THREE.PointLight(HEADLIGHT_COLOR, 0, 20, 2);
               ptLight.name = "AttachedHeadlight";
               node.add(ptLight);
               node.userData.hasLightAttached = true;
            }
          }
        })
      }
  ...
  useFrame((state, delta) => {
    const target = lightsOn ? 15 : 0; // target emissive intensity
    let needsInvalidate = false;
    
    if (processedModel) {
      processedModel.traverse((node) => {
        if (node.isPointLight && node.name === "AttachedHeadlight") {
          const currentLight = node.intensity !== undefined ? node.intensity : 0;
          if (Math.abs(currentLight - target) > 0.01) {
            node.intensity = THREE.MathUtils.lerp(currentLight, target, delta * 5);
        ...
        if (node.isMesh && node.material) {
          ...
          mats.forEach(mat => {
            ...
            if (isTargetLight) {
              // Ensure emissive color is not black if it's supposed to emit light
              if (!mat.emissive || mat.emissive.getHex() === 0) {
                mat.emissive = HEADLIGHT_COLOR;
              }
              const current = mat.emissiveIntensity !== undefined ? mat.emissiveIntensity : 0;
              
              if (Math.abs(current - target) > 0.01) {
                mat.emissiveIntensity = THREE.MathUtils.lerp(current, target, delta * 5);
                needsInvalidate = true;
              } else {
...
function Scene({ modelUrl, isMobile, scrollProgress, lightsOn }) {
  ...
  const scrollPoints = useMemo(() => {
    ...
    const basePoints = [
      ...
      {
        scroll: 2/3,
        position: [15, 8, 12],       // High angle right — reveals interior / far side
        lookAt: [0, 0, 0]
      },
      {
        scroll: 0.75,
...
export default function ModelBackground({ modelUrl, scrollProgress }) {
  const { isDark, carLightsOn } = useTheme()
  ...
  return (
    ...
    >
      ...
      >
        <Scene modelUrl={modelUrl || porsche911} isMobile={mobile} scrollProgress={scrollProgress} lightsOn={carLightsOn} />
      </Canvas>
    </div>
  )
}
